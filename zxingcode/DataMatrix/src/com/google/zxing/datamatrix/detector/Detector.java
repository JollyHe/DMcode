...
/*    float[] pointsX = new float[4];
    ...
    ResultPoint topLeft = new ResultPoint(pointsX[1], pointsY[2]);
    ResultPoint bottomLeft = new ResultPoint(pointsX[0], pointsY[0]);
    ResultPoint bottomRight = new ResultPoint(pointsX[2], pointsY[1]);
    ResultPoint topRight = new ResultPoint(pointsX[3], pointsY[3]);
 */   
    ...
    float Dwidth;
    float Dhight;
    if (Math.abs(topRight.getX() - topLeft.getX()) < Math.abs(topRight.getY() - topLeft.getY())) {
    	steep = true;
    }
    if (Math.abs(bottomRight.getY() - topRight.getY()) < Math.abs(bottomRight.getX() - topRight.getX())) {
    	steep = true;
    }
    if (steep) {
    	Dwidth = Math.abs(bottomRight.getX() - topRight.getX());
        Dhight = Math.abs(topRight.getY() - topLeft.getY());
    }else {
    	Dwidth = Math.abs(topRight.getX() - topLeft.getX());
        Dhight = Math.abs(bottomRight.getY() - topRight.getY());
    }
    if (Dwidth >= 1.5 * Dhight || Dhight >= 1.5 * Dwidth) {
    	isRectangle = true;
    }else {
    	isRectangle = false;
    }
    ...
    //误差纠正
    if (isRectangle) {
    	if (Math.abs(dimensionRight - 8) < 2) {
    		if (Dwidth < 3 * Dhight) {
    			dimensionTop = 18;
    			dimensionRight = 8;
    		}else {
    	...
    	/*if (Math.abs(dimensionRight - 12) <= 2 || Math.abs(dimensionTop - 26) <= 2) {
    		dimensionTop = 26;
			dimensionRight = 12;
    	}*/
    }
   ...
   * 使用类似bresenham算法的方法，计算两点之间的黑白过渡数。
   */
  private int transitionsBetween(ResultPoint from, ResultPoint to) {
    ...
    for (int x = fromX, y = fromY; x != toX; x += xstep) {
        ...
        if (count == 2) {
        	sumBlackAndWhite = Math.abs(x - fromX) + 1;
        	break;
        }
      }
    if (sumBlackAndWhite != 0) {
    	transitions = ((Math.abs(fromX - toX) + 1) / sumBlackAndWhite + 1) * 2;
    }else {
    	transitions = 0;
    }*/
    return transitions;
